...
public class CinemachinePresetToggleByUIImage : MonoBehaviour
{
    public CinemachineVirtualCamera virtualCamera; // Reference to the Cinemachine Virtual Camera
    public Image checklistImage; // Reference to the UI Image component

    // Preset A (values when the Checklist Image is active)
    public float screenX_A = 0.5f;
    public float deadZoneWidth_A = 0.1f;
    public float softZoneWidth_A = 0.8f;
    public float biasX_A = 0f;

    // Preset B (values when the Checklist Image is inactive)
    ...
    void Start()
    {
        if (virtualCamera != null)
        {
            ...
            if (framingTransposer != null)
            {
                ...
                {
                    // Set initial values based on the Checklist Image's active state
                    ApplyCurrentPreset();
                }
                else
                {
                    Debug.LogError("No Checklist Image assigned!");
                }
            }
            else
            {
    ...
    void Update()
    {
        if (checklistImage != null && framingTransposer != null && !isLerping)
        {
            ApplyCurrentPreset();
        }
    }
    ...
    {
        if (checklistImage.gameObject.activeSelf)
        {
